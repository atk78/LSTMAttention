...
def get_tokens(smiles_array: list[str], split_l=1, poly_flag=False):
    """
    SMILES文字列のlistをトークン化する関数

    Parameters
    ----------
    smiles_array : list[str]
        SMILES文字列のlist
    split_l : int, optional
        トークン化された文字列を分割する長さ, by default 1
    poly_flag : bool, optional
        SMILESがPolymerの構造（[*]を二つ持つ）かどうか

    Return
    ----------
        トークン化されたSMILES文字列のlist
    """
...
def smiles_tokenizer(smiles: str, poly_flag=False):
    ...
    smiles : str
        トークン化するSMILES文字列
    poly_flag : bool, optional
        SMILESがPolymerの構造（[*]を二つ持つ）かどうか, by default False
...
def get_token_to_int(tokens: list[str]):
    """
    トークンを整数に変換する辞書を作成する関数
    """
...
def get_int_to_token(tokens: list[str]):
    """
    整数をトークンに変換する辞書を作成する関数
    """
    return dict((i, c) for i, c in enumerate(tokens))


def extract_vocab(lltokens: list[list[str]]):
    """
    トークン化されたSMILES文字列のlistからユニークなトークンを抽出する関数

    Parameters
    ----------
    lltokens : list[list[str]]
        トークン化されたSMILES文字列のlist
...
def add_extract_tokens(tokens: list[str], vocab_size: int):
    """
    トークンのリストに特別なトークンを追加し、語彙サイズを更新する関数
...
def int_vec_encode(
    tokenized_smiles_list: list[list[str]], max_length: int, tokens: list[str]
):
